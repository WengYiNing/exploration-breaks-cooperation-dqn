...
def _generate_simple_d_regular_on_nodes(nodes: np.ndarray, d: int, rng: np.random.Generator, max_tries: int = 200):

    m = len(nodes)
    if m <= d:
        raise RuntimeError(f"Module too small: size={m}, d={d}")
    if (m * d) % 2 == 1:
        raise RuntimeError(f"Module size*d must be even: size={m}, d={d}")

    node_list = nodes.astype(np.int32)

    for _ in range(max_tries):
        neigh = {int(v): set() for v in node_list}

        stubs = np.repeat(node_list, d)
        rng.shuffle(stubs)
        ...
        for i in range(0, len(stubs), 2):
            ...
            if a == b:
                ok = False
                break
            if (b in neigh[a]) or (a in neigh[b]):
                ok = False
                break
        ...
        if all(len(neigh[int(v)]) == d for v in node_list):
            return neigh

    raise RuntimeError(f"Failed to generate simple {d}-regular graph on module after {max_tries} tries.")


def _perfect_matching_across_modules(all_nodes: np.ndarray, module_id: np.ndarray, rng: np.random.Generator, max_tries: int = 200):
    ...
    raise RuntimeError(f"Failed to build cross-module perfect matching after {max_tries} tries.")


def generate_modular_4_regular_neighbors(
    N: int,
    seed: int = 0,
    num_modules: int = 9,
    k_in: int = 3,
    ...
    k_out = k - k_in
    if k_out != 1:
        raise RuntimeError("This stable construction is designed for k_out=1 (i.e., k_in=3).")

    rng = np.random.default_rng(seed)

    nodes = np.arange(N, dtype=np.int32)
    rng.shuffle(nodes)
    modules = np.array_split(nodes, num_modules)

    module_id = np.empty(N, dtype=np.int32)
    for m, arr in enumerate(modules):
        module_id[arr] = m

    neighbors = [set() for _ in range(N)]
    for m in range(num_modules):
        group = modules[m]
        intra = _generate_simple_d_regular_on_nodes(group, d=k_in, rng=rng, max_tries=max_tries)

        for v in group:
            v = int(v)
            neighbors[v].update(intra[v])

    pairs = _perfect_matching_across_modules(np.arange(N, dtype=np.int32), module_id, rng=rng, max_tries=max_tries)

    for a, b in pairs:
        if (b in neighbors[a]) or (a in neighbors[b]) or (a == b):
            raise RuntimeError("Unexpected conflict in cross edges.")
        neighbors[a].add(b)
        neighbors[b].add(a)
    ...
    return np.array([sorted(list(neighbors[i])) for i in range(N)], dtype=np.int32)
...
class PDG_Vectorized:
    def __init__(self, Dr: float, tau_init: float, anneal_step:int):
        ...
        self.neighbor = generate_modular_4_regular_neighbors(
            N,
            seed=SEED,
            num_modules=9,
            k_in=3,
    ...
    def run(self):
        ...
        print(f"Dr: {self.Dr}, SIZE: {N}, SEED: {SEED}, "
...
def sweep_Dr(
    ...
    if tau_init_values is None:
        tau_init_values = [0.127, 0.260, 0.393, 0.527, 0.660, 0.793]
