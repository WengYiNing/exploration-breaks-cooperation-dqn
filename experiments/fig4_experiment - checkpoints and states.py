...
class PDG_Vectorized:
    ...
    def run(self):
        ...
        ckpt_path = (
            f"{ckpt_dir}/sharedDQN_SIZE{SIZE}_SEED{SEED}_Dr{self.Dr}"
            f"_tauinit_{self.policy.tau_init}_taufinal_{self.policy.tau_final}"
            f"_anneal_{self.policy.tau_anneal_steps}_B_{B_mean_tau:.6f}.pt"
        )
        ...
        torch.save(
            {
                ...
                "SIZE": SIZE,
                "SEED": SEED,
                "tau_init": self.policy.tau_init,
                "tau_final": self.policy.tau_final,
                "tau_anneal_steps": self.policy.tau_anneal_steps,
                "B_mean_tau": B_mean_tau,
            },
            ckpt_path,
        )
        ...
        for _ in range(focus_round):
            ...
            self.compute_states()     
            self.compute_rewards() 
            collected_states.append(self.last_state.copy())
        ...
        flat_states = collected_states.reshape(-1, self.new_state.shape[1])  
        n_sample = 150
        idx = np.random.choice(flat_states.shape[0], n_sample, replace=False)
        sampled_states = flat_states[idx]

        save_dir = "states_95k_100k"
        os.makedirs(save_dir, exist_ok=True)

        state_path = (
            f"{save_dir}/state_Size{SIZE}_SEED{SEED}_Dr{self.Dr}_tauinit_{self.policy.tau_init}"
            f"_B_{B_mean_tau:.6f}.npy"
        )

        np.save(state_path, sampled_states)
