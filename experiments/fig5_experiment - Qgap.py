    ...
    def run(self):
        ...
        with torch.no_grad():
            for _ in range(focus_round):
                ...
                states_t = torch.from_numpy(self.last_state).float().to(self.policy.device)
                q = self.policy.model(states_t)  
        
                top2, _ = torch.topk(q, k=2, dim=1)
                gaps = top2[:, 0] - top2[:, 1]
                qgap_list.append(gaps.mean().item())

        final_qgap_avg = float(np.mean(qgap_list))
        ...
        filename_qgap = (
            f"results/final_qgap_SIZE{SIZE}_SEED{SEED}_Dr{self.Dr}"
        ...
        np.savetxt(filename_qgap, np.array([final_qgap_avg], dtype=np.float32), fmt="%.6f")
