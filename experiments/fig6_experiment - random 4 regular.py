...
def generate_random_4_regular_neighbors(N: int, seed: int = 0, max_tries: int = 200) -> np.ndarray:
    rng = np.random.default_rng(seed)

    for _ in range(max_tries):
        stubs = np.repeat(np.arange(N, dtype=np.int32), 4)
        rng.shuffle(stubs)
        ...
        if all(len(neighbors[i]) == 4 for i in range(N)):
            return np.array([sorted(list(neighbors[i])) for i in range(N)], dtype=np.int32)

    raise RuntimeError(f"Failed to generate a simple random 4-regular graph after {max_tries} tries.")
...
class PDG_Vectorized:
    def __init__(self, Dr: float, tau_init: float, anneal_step:int):
        ...
        self.neighbor = generate_random_4_regular_neighbors(N, seed=SEED)
