...
class PDG_Vectorized:
    def __init__(self, Dr: float, tau_init: float, anneal_step:int):
        ...
        self.policy = SharedDQN(state_size=7, tau_init=tau_init, tau_anneal_steps=anneal_step)
        ...
        self.last_state = np.zeros((N, 7), dtype=np.float32)
        self.new_state  = np.zeros((N, 7), dtype=np.float32)
    ...
    def compute_states(self):
        ...
        progress = min(1.0, (self.policy.env_steps + 1) / max(1, self.policy.tau_anneal_steps))
        self.new_state[:, 6] = progress  
