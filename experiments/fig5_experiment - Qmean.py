...
class PDG_Vectorized:
    ...
    def run(self):
        ...
        for _ in range(focus_round):
            ...
            with torch.no_grad():
                states_t = torch.from_numpy(self.last_state).float().to(self.policy.device, non_blocking=True)
                q = self.policy.model(states_t)
            
                valuelevel_sum += float(q.mean(dim=1).sum().item())
                valuelevel_cnt += int(q.shape[0])

            current_coop_rate = self.compute_coop_rate()
            self.cooperation_rates.append(current_coop_rate)
            cooper_cnt += current_coop_rate

        avg_valuelevel = (valuelevel_sum / valuelevel_cnt) if valuelevel_cnt > 0 else float("nan")
        ...
        filename_valuelevel = (
            f"results/valuelevel_eval_SIZE{SIZE}_SEED{SEED}_Dr{self.Dr}"
        ...
        np.savetxt(
            filename_valuelevel,
            np.array([avg_valuelevel], dtype=np.float32),
            fmt="%.6f"
        )
