...
class SharedDQN:
    def __init__(
        ...
        self.memory_agent = np.empty((memory_size,), dtype=np.int32)  
...
class PDG_GroupShared:
    def __init__(self, Dr: float, tau_init: float, anneal_step:int):
        ...
        rng.shuffle(all_ids)
        groups = np.array_split(all_ids, 10) 
        self.group_indices = [g.astype(np.int32) for g in groups]

        self.num_groups = 10
        self.group_size = len(self.group_indices[0]) 

        self.policies = []
        for _ in range(self.num_groups):
            pol = SharedDQN(
                N_env=self.group_size,
                tau_init=tau_init,
                tau_final=0.10,           
    ...
    def compute_rewards(self):
        ...
        sep_reward = self.payoff[me[:, None], neighbor_new_actions]
        self.rewards = sep_reward.mean(axis=1).astype(np.float32)
    ...
    def run(self):
        ...
        for step in range(total_round - focus_round):
            ...
            actions = np.empty_like(self.new_action)
            for g, idx in enumerate(self.group_indices):
                a_g = self.policies[g].action_batch(self.last_state[idx])
                actions[idx] = a_g.astype(np.int8)
            ...
            for g, idx in enumerate(self.group_indices):
                pol = self.policies[g]
                pol.remember_batch(self.last_state[idx], self.new_action[idx], self.rewards[idx], self.new_state[idx])
                if (step % 4 == 0):
                    pol.replay()
                pol.on_env_step()
        ...
        for _ in range(focus_round):
            ...
            actions = np.empty_like(self.new_action)
            for g, idx in enumerate(self.group_indices):
                action_g = self.policies[g].action_batch(self.last_state[idx])
                actions[idx] = action_g.astype(np.int8)
            self.new_action = actions

            self.compute_states()
            self.compute_rewards()

            cr = self.compute_coop_rate()
            self.cooperation_rates.append(cr)
            cooper_cnt += cr

        K = int(self.policies[0].tau_anneal_steps // 2)
        tau_log0 = self.policies[0].tau_log
        B_mean_tau = float(np.mean(tau_log0[:K])) if len(tau_log0) >= K else float(np.mean(tau_log0))

        avg_focus = cooper_cnt / focus_round
        print(f"[GroupShared 10] Dr: {self.Dr}, SIZE: {SIZE}, SEED: {SEED}, "
              f"group_size: {self.group_size}, groups: {self.num_groups}, "
              f"tau_init: {self.policies[0].tau_init}, tau_final: {self.policies[0].tau_final}, "
              f"anneal: {self.policies[0].tau_anneal_steps}, B:{B_mean_tau:.6f}, result: {avg_focus:.6f}")

        os.makedirs("results", exist_ok=True)
        filename = (
            f"results/coop_GROUP10_SIZE{SIZE}_SEED{SEED}_Dr{self.Dr}"
            f"_tauinit_{self.policies[0].tau_init}_taufinal_{self.policies[0].tau_final}"
            f"_anneal_{self.policies[0].tau_anneal_steps}_B_{B_mean_tau:.6f}.txt"
        )
